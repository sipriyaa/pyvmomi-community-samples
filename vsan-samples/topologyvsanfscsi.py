...
def getClusterInstance(clusterPath, serviceInstance):
    """
    Retrieve the cluster instance using the full cluster path.
    """
    content = serviceInstance.RetrieveContent()
    searchIndex = content.searchIndex

    # Use FindByInventoryPath to locate the cluster directly by its path
    cluster = searchIndex.FindByInventoryPath(clusterPath)
    if cluster and isinstance(cluster, vim.ClusterComputeResource):
        return cluster
    else:
        raise ValueError(f"Cluster not found or invalid cluster type: {clusterPath}")
...
def main():
   ...
   ovfUrl = "https://build-squid.vcfd.broadcom.net/build/mts/release/bora-24762657/publish/vdfs-fsvm/VMware-vSAN-File-Services-Appliance-9.1.0.0.24762657_OVF10.ovf"
   if not ovfUrl:
